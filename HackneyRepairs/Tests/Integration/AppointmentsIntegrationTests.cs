...
namespace HackneyRepairs.Tests.Integration
{
    public class AppointmentsIntegrationTests
    {
        ...
        [Fact]
        public async Task return_a_200_result_for_valid_appointment_requests()
        {
        ...
        [Fact]
        public async Task return_a_400_result_for_empty_appointment_request_parameter_string()
        {
            var result = await _client.GetAsync("v1/work_orders/ /appointments");
            Assert.Equal(HttpStatusCode.BadRequest, result.StatusCode);
        }

        [Fact]
        public async Task return_a_500_result_when_there_is_an_internal_server_error_for_appointment_requests()
        {
            var result = await _client.GetAsync("v1/work_orders/01550853/appointments");
            Assert.Equal(HttpStatusCode.InternalServerError, result.StatusCode);
        }

        [Fact]
        public async Task return_a_json_object_for_valid_appointment_requests()
        {
            var result = await _client.GetAsync("v1/work_orders/01550854/appointments");
            string result_string = await result.Content.ReadAsStringAsync();
            StringBuilder json = new StringBuilder();
