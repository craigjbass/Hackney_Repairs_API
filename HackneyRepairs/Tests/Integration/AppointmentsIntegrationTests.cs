...
namespace HackneyRepairs.Tests.Integration
{
	public class AppointmentsIntegrationTests
	{
		...
		[Fact]
		public async Task return_list_of_UhtAppointmentEntities_json_object_with_200()
		{
			var result = await _client.GetAsync("v1/work_orders/01550854/appointments");
			var jsonresult = await result.Content.ReadAsStringAsync();
			var appointments = JsonConvert.DeserializeObject<List<UhtAppointmentEntity>>(jsonresult).ToList();

			Assert.IsType<List<UhtAppointmentEntity>>(appointments);
			Assert.Equal(HttpStatusCode.OK, result.StatusCode);
			Assert.Equal("application/json", result.Content.Headers.ContentType.MediaType);
		}
